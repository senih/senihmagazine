...
//
// 15/01/2008; Kosta Mihajlov; Added POSSystemBMP method
// 25/02/2008; Kosta Mihajlov; Added Method for merging two pdf documents- MergePDF
// Notes:
...
namespace Magazine
{
    ...
    public class GhostScript : IDisposable
    {
        ...
        /// <summary>
        /// Processing Started Event Arguments.
        /// </summary>
        ...
        public class ProcessingPageEventArgs : EventArgs
        {
            ...
            /// <summary>
            /// Processing Page Event Arguments.
            /// </summary>
        ...
        public class ProcessingCompletedEventArgs : EventArgs
        {
            ...
            /// </summary>
            /// <param name="ouputFilenames">Output File Names.</param>
            /// <param name="pageCount">Page Count.</param>
            /// <returns></returns>
        ...
        public enum OutputDevice
        {
            ...
            /// See description at:
            /// <see href="http://ghostscript.com/doc/8.54/Devices.htm#TIFF"/>
            /// </summary>
            tiffsep,
            /// <summary>
        ...
        /// <summary>
        /// Represents the gsapi New Instance
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="GhostScript"/> class.
        /// </summary>
        private GhostScript()
        {
        }
        ...
        /// <summary>
        /// Gets the gsapi_revision_t version.
        /// </summary>
        public gsapi_revision_t GetVersion()
        {
            ...
            //Veljo commented the unreachable code
            //return new gsapi_revision_t();
        ...
        public List<string> Convert(OutputDevice outputDevice, DeviceOption[] deviceOptions, string inputFileName, string outputPath, string outputFileName, string temporaryFileFolder, int resolution)
        {
            ...
            File.Copy(inputFileName, temporaryInputFilename, true);
            ...
            try
            {
                ...
                while (File.Exists(outputFileNameCheck))
                {
				...
				//Kosta: Added this code to include the case where the output is strongly specified
				if (outputFilenames.Count == 0)
				{
					if (File.Exists(Path.Combine(outputPath, outputFileName)))
					{
						outputFilenames.Add(Path.Combine(outputPath,outputFileName));
					}
				}
            ...
            finally
            {
                ...
                if (File.Exists(temporaryInputFilename))
                {
                    try
                    {
                        File.Delete(temporaryInputFilename);
                    }
                    catch
                    {
        ...
        /// The output files will be saved with the following naming convention:<br />
        ...
        /// <param name="resolution"></param>
        /// <returns></returns>
        public List<string> CreateColorSeparations(string inputFileName, string outputPath, string temporaryFileFolder, int resolution)
        {
            InitializePrivateMembers();
        ...
        public class DeviceOptions
        {
            ...
            /// <returns></returns>
            public static DeviceOption[] jpg(int quality, int txtAntialias, int graphAntialias)
            {
                DeviceOption Quality = new DeviceOption("-dJPEGQ=", quality.ToString());
                DeviceOption TextAntialiasing = new DeviceOption("-dTextAlphaBits=", txtAntialias.ToString());
                DeviceOption GraphAntialiasing = new DeviceOption("-dGraphicsAlphaBits=", graphAntialias.ToString());

                DeviceOption[] Options = new DeviceOption[3];
                Options.SetValue(Quality, 0);
                Options.SetValue(TextAntialiasing, 1);
                Options.SetValue(GraphAntialiasing, 2);
            ...
            /// <param name="paperFormat">Paper Format</param>
            /// <returns></returns>
            public static DeviceOption[] POSSystemBMP(int firstPage, int lastPage, string resolution, string paperFormat)
            {
                DeviceOption FirstPage = new DeviceOption("-dFirstPage=", firstPage.ToString());
                DeviceOption LastPage = new DeviceOption("-dLastPage=", lastPage.ToString());
                DeviceOption Resolution = new DeviceOption("-r", resolution);
                DeviceOption PaperSize = new DeviceOption("-spapersize=", paperFormat);

                DeviceOption[] Options = new DeviceOption[4];
                Options.SetValue(FirstPage, 0);
                Options.SetValue(LastPage, 1);
                Options.SetValue(Resolution, 2);
                Options.SetValue(PaperSize, 3);
                return Options;
            }
            /// <summary>
        ...
        private static string GetGSErrorMessage(int returnCode)
        {
            switch (returnCode)
            {
                ...
                case (int)ReturnCode.e_stackunderflow: return "level 1 error: e_stackunderflow";
